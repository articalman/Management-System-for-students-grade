...
// 排序函数 n=1升序 n=0降序  m 选择排序参数
void Menu();  // 打印系统界面
void about();  //显示关于页面
void FileOutput(student *L); // 保存到文件
void FileInput(student *&L); // 从文件中读取数据

int main(){
    student *head;
    InitList(head);
    system("title 学生成绩管理系统V3.0");//设置控制台标题
...
void Sort(student *&L , int n , int m){  // 采用冒泡排序
...
void about(){
    cout << "\t----------------新增内容----------------\n\n";
    cout << "\t1.界面美化(Menu函数)\n";
    cout << "\t2.delete函数完善删除条件多样性\n";
    cout << "\t3.增加新功能(排序函数)\n";
    cout << "\t4.增加显示补考名单(<60)和优秀名单(>90)\n";
    cout << "\t5.在界面内容标注对应的函数及负责人\n";
    cout << "\t6.流程图和文档\n";
    cout << "\t7.文件的读写操作\n";
}
...
void FileOutput(student *L){
    FILE *fp = NULL;
    if((fp = fopen("output.txt", "w+")) == NULL){// 自动生成，不用创建
        cout << "Can not open this file!\n";  // 读写错误
    }
    ofstream outfile("output.txt", ios::app);  
    //以追加的方式录入信息，直接将信息追加到以前文件的末尾

    outfile << "姓名    学号    语文    数学    英语    专业课    总成绩\n";
    while (L->next != NULL){
        L = L->next;
        outfile << L->name << "\t\t";
...
void FileInput(student *&L){
    ...
    // 打开一个文件，允许读写，如果没有则创建一个
    ifstream inputfile("input.txt",ios::in); // 读取文件中的内容
